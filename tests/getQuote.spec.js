...
test.describe.serial("List test cases get quote - Non login view", () => {
  ...
  test("Get Quote widget with QM Pricing - Comapny website", async ({ page }) => {
    ...
    await expect(page.locator("//div[@id='com.mvu.loan.client.quote.QuoteResult']//input[@aria-label='Row 1 Compare']")).toBeVisible();
  ...
  test("Get quote widget with Non-QM Pricing - Company website", async ({ page }) => {
    ...
    await page.waitForTimeout(12000);
    await expect(page.locator("//div[@id='com.mvu.loan.client.quote.NonQMQuoteResult']//div[@class='table-row thead']")).toBeVisible();
